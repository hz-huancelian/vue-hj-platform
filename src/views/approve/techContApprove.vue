<template>
  <div class="app-container">
      ...
      <el-row>
        <el-col :span="20">
          ...
          <el-form-item label="评审状态:">
            <el-select v-model="searchForm.recordStatus" clearable style="width:160px" filterable placeholder="评审状态">
              <el-option
                v-for="item in Object.keys(APPROVE_STATUS)"
                :key="item"
                :label="APPROVE_STATUS[item]"
                :value="item"
              />
            </el-select>
          </el-form-item>
    ...
    <div>
      ...
      >
        ...
        <el-table-column label="评审状态" align="center">
          <template slot-scope="{row}">
            <el-tag effect="dark" :type="row.judgeRecordStatus | statusQualityFilter">
        ...
        <el-table-column fixed="right" align="center" label="操作" width="120">
          <template slot-scope="{row}">
            ...
            <div v-if="row.judgeRecordStatus === '0'" class="erc-divider-vertical" />
            <el-button v-if="row.judgeRecordStatus === '0'" type="text" @click="handleApprove(row)"> 合同评审</el-button>
    ...
    <el-dialog title="评审判定" :visible.sync="approveVisible" center>
...
export default {
  ...
  methods: {
    ...
    handleSearch() {
      judgeServer.findJudgeTaskForDeptManagerByCondition(this.searchForm).then(res => {
        if (res.code === 200) {
    ...
    handleApprove(row) {
      this.approveForm.judgeRecordId = row.judgeRecordId
    ...
    confirmDialog() {
      this.$confirm('确定提交?', '提示', {
        ...
        judgeServer.doAuditRecord(this.approveForm).then(res => {
          if (res.code === 200) {
            this.$notify({
              title: '成功',
              message: res.message,
