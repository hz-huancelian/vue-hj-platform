  ...
  <div class="app-container">
        ...
        <el-col :span="20">
          ...
          <el-form-item label="任务状态:">
            <el-select v-model="searchForm.checkTaskStatus" clearable filterable style="width:165px" placeholder="请选择状态">
              <el-option
                v-for="item in Object.keys(FACTOR_STATUS)"
                :key="item"
                :label="FACTOR_STATUS[item]"
                :value="item"
              />
            </el-select>
          </el-form-item>
    ...
    <div>
      ...
      >
        ...
        <el-table-column prop="checkFactorCount" label="检测因子数" align="center" />
        ...
        <el-table-column prop="checkTaskStatus" label="状态" align="center">
          <template slot-scope="{row}">
            <el-tag effect="dark" :type="row.checkTaskStatus | statusQualityFilter">
              {{ FACTOR_STATUS[row.checkTaskStatus] }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="检测进度" width="120" align="center">
          <template slot="header">
            检测进度 <i class="el-input__icon " />
            <el-popover
              placement="top-start"
              title=""
              width="300"
              trigger="hover"
              content="该任务经实验室负责人审核通过的样品数/该任务采集的总样品数"
            >
              <svg-icon slot="reference" icon-class="help" style="font-size:16px" />
            </el-popover>
          </template>
          <template slot-scope="{row}">
            {{ row.completeCheckFactorCount }}/{{ row.checkFactorCount }}
          </template>
        </el-table-column>

        <el-table-column fixed="right" align="center" label="操作" width="140px">
          <template slot-scope="scope">
            <el-button type="text" @click="handleSampleInfo(scope.row)"> 因子列表</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    handleSearch() {
      checkServer.findCheckTaskByCondition(this.searchForm).then(res => {
        if (res.code === 200) {
    ...
    handleSampleInfo(row) {
      this.$router.push(`/other/laboratorTask/${row.checkTaskId}`)
    }
