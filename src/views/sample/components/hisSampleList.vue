  ...
  <div>
    ...
    <div>
      ...
      >
        ...
        <el-table-column prop="dayAndCount" label="频次(次/天)" width="130px" align="center" />
        ...
        <el-table-column label="采样时间" align="center" width="190px">
        ...
        <el-table-column prop="sampTaskStatus" label="状态" align="center">
          <template slot-scope="{row}">
            <el-tag effect="dark" :type="row.sampleStatus | statusQualityFilter">
              {{ SAMPLE_STATUS[row.sampleStatus] }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog title="现场采样记录单下载" :visible.sync="downVisible" center>
      <div style="width: 500px;margin:0 auto">
        ...
        <el-form :model="recordForm" label-width="130px">
          ...
          <el-form-item label="采样现场记录时间:">
            <el-date-picker
              v-model="recordForm.sampleDate"
              type="date"
              placeholder="采样现场记录时间"
              value-format="yyyy-MM-dd"
              style="width:300px"
            />
          </el-form-item>
...
export default {
  ...
  methods: {
    handleSearch() {
      sampleServer.findHisSampItemByCondition(this.searchForm).then(res => {
        if (res.code === 200) {
    ...
    confirmDownload() {
      if (this.recordForm.taskId && this.recordForm.secClassId && this.recordForm.sampleDate) {
        const url = '/api/word/downSampleRecordByTaskId?taskId=' + this.recordForm.taskId + '&secdClassId=' + this.recordForm.secClassId + '&sampleDate=' + this.recordForm.sampleDate
        window.open(url)
