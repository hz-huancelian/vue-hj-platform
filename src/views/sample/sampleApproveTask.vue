      ...
      >
        ...
        <el-table-column label="任务周期" align="center" width="190px">
          <template slot="header">
            任务周期 <i class="el-input__icon " />
            <el-popover
              placement="top-start"
              title=""
              width="300"
              trigger="hover"
              content="任务调度时录入的任务开始日期和结束日期"
            >
        ...
        <el-table-column prop="sampTaskStatus" label="采样进度" align="center">
          <template slot="header">
            采样进度 <i class="el-input__icon " />
            <el-popover
              placement="top-start"
              title=""
              width="300"
              trigger="hover"
              content="该任务已经入库的样品数/该任务所需采样的总样品数"
            >
              <svg-icon slot="reference" icon-class="help" style="font-size:16px" />
            </el-popover>
          </template>
        ...
        <el-table-column fixed="right" align="center" label="操作" width="140px">
          <template slot-scope="scope">
            <el-button type="text" @click="handleDetail(scope.row)"> 审核样品</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    handleSearch() {
      sampleServer.findSampTaskForManagerByCondition(this.searchForm).then(res => {
        if (res.code === 200) {
    ...
    handleDetail(row) {
      this.$router.push(`/other/masterApproveList/${row.sampTaskId}`)
    }
  }
}
</script>
