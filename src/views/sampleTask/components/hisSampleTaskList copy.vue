      ...
      >
        ...
        <el-table-column fixed="right" align="center" label="操作" width="140px">
          <template slot-scope="scope">
            ...
            <el-button type="text" @click="handlePrint(scope.row)"> 标签打印</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
          ...
          <td colspan="4" style="text-align: left;padding-left:15px;">
            <el-row v-for="(item, index) in factorData" :key="index">
              ...
              <el-col v-for="el in item.factList" :key="el.id" :span="24">
                {{ el.factorPoint + '（'+ el.dayCount +'*'+ el.frequency+'）: ' }}{{ el.showFactName }}
              </el-col>
            </el-row>
          </td>
        </tr>
...
export default {
  ...
  methods: {
    handleSearch() {
      sampleServer.findHistoryTaskByCondition(this.searchForm).then(res => {
        if (res.code === 200) {
    ...
    doProcessData(data) {
      ...
      if (data.length > 0) {
        ...
        data.forEach(i => {
          const o = parentArray.find(j => j.secdClassName === i.secdClassName)
          if (!o) {
            parentArray.push({
          ...
          if (factMap.get(i.secdClassName)) {
            const ls = factMap.get(i.secdClassName)
            ls.push(i)
          } else {
            const li = []
            li.push(i)
            factMap.set(i.secdClassName, li)
          }
        })

        if (parentArray) {
          parentArray.forEach(e => {
            const flist = factMap.get(e.secdClassName)
            const nList = []
            if (flist) {
              flist.forEach(j => {
